 ...
 * CRUD for subjects (per college/course/branch).
 */
...
exports.list = async (req, res) => {
  try {
    ...
    let sql = `SELECT s.id, s.college_id, s.course_id, s.branch_id, s.name, s.code, s.is_active,
...
exports.create = async (req, res) => {
  try {
    const { college_id, course_id, branch_id, name, code } = req.body;
    if (!college_id || !course_id || !name) {
      return res.status(400).json({ success: false, message: 'college_id, course_id, name required' });
    }
    const scope = req.userScope || {};
    ...
    const [r] = await masterPool.query(
      `INSERT INTO subjects (college_id, course_id, branch_id, name, code) VALUES (?, ?, ?, ?, ?)`,
      [college_id, course_id, branch_id || null, name, code || null]
    );
    res.status(201).json({ success: true, data: { id: r.insertId, college_id, course_id, branch_id: branch_id || null, name, code: code || null } });
  } catch (error) {
...
exports.update = async (req, res) => {
  try {
    ...
    const [existing] = await masterPool.query(
      `SELECT id FROM subjects s WHERE s.id = ? AND (${condition})`,
      [id, ...params]
    );
